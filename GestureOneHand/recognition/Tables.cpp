...
ULSet*
TwoDTable::getCntx(unsigned long event)
{
...
double
TwoDTable::get(unsigned long event, unsigned long context)
{
...
void
TwoDTable::add(unsigned long context, unsigned long event, double count)
{
    TwoDTable::iterator f = find(context);
    OneDTable* entry=0;
    if (f == end()) {
...
void
TwoDTable::load(istream& file, Str2IdMap& str2id)
{
    string name1, name2;
    double c;
...
void
TwoDTable::save(ostream& file, Str2IdMap& str2id)
{
    for (TwoDTable::iterator i = begin(); i!=end(); i++) {
...
bool
TwoDTable::rand(unsigned long curr, unsigned long& next)
{
...
void
OneDTable::load(istream& file, Str2IdMap& str2id)
{
    string name;
    double c;
    while (file>>name>>c) {
...
void
OneDTable::save(ostream& file, Str2IdMap& str2id)
{
...
double
OneDTable::get(unsigned long event)
{
    OneDTable::iterator f = find(event);
    if (f == end()) return smoothedZeroCount();
    else return f->second;
}

void
OneDTable::add(unsigned long event, double count)
{
    OneDTable::iterator f = find(event);
    if (f == end()) (*this)[event] += count;
    else f->second = sumLogProb(f->second, count);
}

bool
OneDTable::rand(unsigned long& next)
{
