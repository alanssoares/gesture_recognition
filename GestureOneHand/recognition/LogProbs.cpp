...
double
sumLogProb(vector<double>& logprobs)
{
    double max = 0;
    unsigned int i;
    ...
    // the largest probability is 0 (log prob= -inf)
    if (isinf(max)) return max;   // return log 0
...
double
sumLogProb(double logprob1, double logprob2)
{
