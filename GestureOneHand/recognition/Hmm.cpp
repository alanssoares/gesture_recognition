...
double
Hmm::getTransProb(Transition* trans)
{
    return _transition.get(trans->_to->state(), trans->_from->state());
}

double
Hmm::getEmitProb(Transition* trans)
{
...
double
Hmm::viterbi(vector<Transition*>& path)
{
    ...
    // reverse the path
    for (long i = 0, j = path.size()-1; i<j; i++, j--) {
...
Hmm::backward()
{
    unsigned long T = _timeSlots.size()-1;
    
    if (T<1) return;// no observation
...
Hmm::obsProb()
{
    if (_timeSlots.size() < 1) return 1; // no observations
...
double
Hmm::getPseudoCounts(PseudoCounts& counts)
{
...
void
Hmm::baumWelch(vector<vector<unsigned long>*>& sequences, int iterations)
{
    cerr << "Training with Baum-Welch for up to " << iterations << " iterations, using " << sequences.size() << " sequences." << endl;
...
void
Hmm::updateProbs(PseudoCounts& counts)
{
    _transition.clear();
    _emission.clear();
...
void
Hmm::addObservation(unsigned long o)
{
...
void
Hmm::readSeqs(istream& istrm, vector<vector<unsigned long>*>& sequences)
{
    string line;
    const string delims(" ");
...
void
Hmm::genSeqs(ostream& ostrm, int seqs)
{
...
void
Hmm::genSeq(vector<unsigned long>& seq)
{
...
void
PseudoCounts::print(Str2IdMap& str2id)
{
